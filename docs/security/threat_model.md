...
| K-02 | Sin certificate pinning en Dio | I | 🟡 Medio | Baja | ✅ | **Decisión definitiva: no implementar.** El backend usa CA pública (Cloudflare). El pinning añade complejidad operativa sin beneficio significativo para este perfil de amenaza. La app confía en el TrustManager del OS (Android) y ATS (iOS). |
